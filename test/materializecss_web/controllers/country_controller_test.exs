defmodule MaterializecssWeb.CountryControllerTest do
  use MaterializecssWeb.ConnCase

  alias Materializecss.Countries

  @create_attrs %{abbrv: "some abbrv", active_status: true, del_status: true, name: "some name"}
  ...
  describe "index" do
    test "lists all countires", %{conn: conn} do
      conn = get(conn, Routes.country_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing Countires"
    end
  end
  ...
  describe "update country" do
    ...
    test "redirects when data is valid", %{conn: conn, country: country} do
      ...
      assert html_response(conn, 200) =~ "some updated abbrv"
    end
